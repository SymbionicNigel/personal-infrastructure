...
- **i7 bootstrap:** one unavoidable local touch — flash Ubuntu LTS, run a
  ...
  `subnet-router`.
- **Local infra lives in a new top-level `homelab/` dir**, parallel to `linode/`:
  the Ansible tree (`homelab/ansible/`) plus shared provisioning scripts
  (`homelab/scripts/`). A single parameterized `install-tailscale.sh` is the one
  source of truth for joining a box to the tailnet, invoked by the cloud host's
  Terraform `null_resource`, the Ansible `tailscale` role, and the flash-drive
  `bootstrap.sh` alike.
